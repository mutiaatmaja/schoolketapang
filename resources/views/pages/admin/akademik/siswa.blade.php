...
new class extends Component {
    ...
    public function rules(): array
    {
        return [
            ...
            'nik' => ['nullable', 'string', 'max:30', Rule::unique('students', 'nik')->ignore($this->editingId)],
        ...
        try {
            ...
            if ($import->processedRows() === 0) {
                throw ValidationException::withMessages([
                    'importFile' => 'Tidak ada baris siswa yang berhasil diproses. Gunakan template import dan pastikan data terisi.',
                ]);
            }
            ...
            $this->dispatch('toast', type: 'error', message: collect($exception->errors())->flatten()->first() ?? 'Import siswa gagal.');
    ...
    <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-500">Data siswa mengacu ke kelas yang sudah dibuat pada modul kelas.
                </p>
            </div>
        ...
        <div class="relative mt-4">
            <input type="text" wire:model.live.debounce.300ms="search"
                placeholder="Cari nama, NIS, NIK, status, atau kelas"
            ...
            <div class="overflow-x-auto">
                <table class="w-full min-w-[1024px] text-left text-sm">
                    ...
                    <tbody>
                        @forelse ($this->students as $student)
                            <tr wire:key="student-{{ $student->id }}" class="border-t border-slate-100 align-top">
                                <td class="px-4 py-3">
                                    ...
                                    <p class="mt-1 text-xs text-slate-500">NIS {{ $student->nis }} · NIK
                                        {{ $student->nik }}</p>
                                </td>
                                ...
                                <td class="px-4 py-3">
                                    <span
                                        class="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-700">{{ $student->status }}</span>
                                </td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-2">
                                        <button type="button" wire:click="openEdit({{ $student->id }})"
                                            wire:loading.attr="disabled" wire:target="openEdit({{ $student->id }})"
                                            class="rounded-lg bg-amber-100 px-3 py-2 text-xs font-semibold text-amber-700 disabled:opacity-50">
                                            <span wire:loading.remove
                                                wire:target="openEdit({{ $student->id }})">Edit</span>
                                            <span wire:loading
                                                wire:target="openEdit({{ $student->id }})">Membuka...</span>
                                        </button>
                                        <button type="button" wire:click="confirmDelete({{ $student->id }})"
                                            ...
                                            <span wire:loading.remove
                                                wire:target="confirmDelete({{ $student->id }})">Hapus</span>
                                            <span wire:loading
                                                wire:target="confirmDelete({{ $student->id }})">Menyiapkan...</span>
                                        </button>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-4 py-8 text-center text-slate-500">Belum ada data siswa.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="fixed inset-0 flex items-start justify-center overflow-y-auto bg-slate-900/50 px-4 py-6 sm:py-10"
            ...
            <div class="w-full max-w-5xl max-h-[calc(100vh-3rem)] overflow-y-auto rounded-3xl bg-white p-6 shadow-xl">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <h2 class="text-lg font-bold text-slate-800">{{ $editingId ? 'Edit Siswa' : 'Tambah Siswa' }}
                        </h2>
                        <p class="mt-1 text-sm text-slate-500">Lengkapi identitas siswa, relasi kelas, dan data orang
                            tua/wali.</p>
                    </div>
                ...
                <form wire:submit="save" class="mt-6 space-y-5">
                    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                        ...
                        <div>
                            ...
                            <select wire:model="schoolClassId"
                                class="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="fatherOccupation"
                                class="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm" />
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="motherOccupation"
                                class="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm" />
            ...
            <div class="w-full max-w-xl rounded-3xl bg-white p-6 shadow-xl">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">Gunakan template agar nama kolom dan format tanggal
                            sesuai.</p>
                    </div>
                    <button type="button" wire:click="closeModal"
                        class="text-slate-400 hover:text-slate-600">Tutup</button>
                ...
                <div
                    ...
                    Pastikan kolom <span class="font-semibold text-slate-800">kelas</span> berisi angka 1 sampai 6 dan
                    status menggunakan AKTIF, LULUS, atau KELUAR.
                </div>
                ...
                <form wire:submit="importStudents" class="mt-5 space-y-4">
                    ...
                    <div class="flex justify-end gap-3">
                        <button type="button" wire:click="closeModal"
                            class="rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-600">Batal</button>
                        <button type="submit" wire:loading.attr="disabled" wire:target="importStudents"
                            class="rounded-xl bg-amber-500 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">
